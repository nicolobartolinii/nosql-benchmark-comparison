...
function run_ycsb_tests() {
    ...
    for (( k=0; k < num_selected_indices; k++ )); do
        ...
        echo "INFO: Starting Scenario ${current_user_scenario_num}: RC=${RC}, OC=${OC}, FC=${FC}, FL=${FL}, RAF=${RAF}, Threads=${THREADS}"
        ...
        for workload_name in "${DEFAULT_WORKLOADS[@]}"; do
            ...
            for (( rep_num=1; rep_num<=${REPETITIONS}; rep_num++ )); do
                ...
                echo "DEBUG: Full command (approximate): docker run --rm --name ... ${YCSB_IMAGE_NAME} ${YCSB_MONGO_SCRIPT_PATH_IN_CONTAINER} ${workload_name} ${rep_num} ${RC} ${OC} ${FC} ${FL} ${RAF} ${DB_USER_SUBDIR} ${THREADS}"
                ...
                docker run --rm --name "${CONTAINER_NAME}" \
                    ...
                    --env-file .env \
                    "${YCSB_IMAGE_NAME}" \
                    "${YCSB_MONGO_SCRIPT_PATH_IN_CONTAINER}" \
                    "${workload_name}" \
                    "${rep_num}" \
                    "${RC}" \
                    "${OC}" \
