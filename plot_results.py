...
DB_ORDER = ['mongo', 'cassandra', 'redis'] # Consistent order for databases
WORKLOAD_ORDER = ['workloada', 'workloadb', 'workloadc', 'workloadd', 'workloade', 'workloadf']

SCENARIO_ORDER = [SCENARIO_PARAMS_TO_NAME_MAP[k][0] for k in sorted(SCENARIO_PARAMS_TO_NAME_MAP.keys())]

# Regex to parse scenario parameters from the directory name
SCENARIO_PATTERN = re.compile(
    r"rc(?P<recordcount>\d+)_"
    r"oc(?P<operationcount>\d+)_"
    r"fc(?P<fieldcount>\d+)_"
    r"fl(?P<fieldlength>\d+)_"
    r"raf(?P<readallfields>true|false)"
)

# Regex to parse file names
FILE_PATTERN = re.compile(
    r"(?P<phase>load|run)_"
    r"rep(?P<repetition>\d+)_"
    r"(?P<timestamp>\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})"
    r"\.txt"
)

# Regex to extract metrics from file content
METRICS_PATTERNS = {
...
# Latency metrics definitions for plotting
WORKLOAD_PRIMARY_LATENCIES = {
    'workloada': [('Read Avg Latency (µs)', 'read_avg_latency_us'), ('Update Avg Latency (µs)', 'update_avg_latency_us')],
    'workloadb': [('Read Avg Latency (µs)', 'read_avg_latency_us')],
    'workloadc': [('Read Avg Latency (µs)', 'read_avg_latency_us')],
    'workloadd': [('Read Avg Latency (µs)', 'read_avg_latency_us')],
    'workloade': [('Scan Avg Latency (µs)', 'scan_avg_latency_us')],
...
WORKLOAD_LATENCY_PERCENTILES = {
    ...
    'workloade': [ 
        {'name': 'Scan Latency (µs)', 'avg': 'scan_avg_latency_us', 'p95': 'scan_95th_latency_us', 'p99': 'scan_99th_latency_us'}
    ],
    'workloadf': [
        {'name': 'Read Latency (µs)', 'avg': 'read_avg_latency_us', 'p95': 'read_95th_latency_us', 'p99': 'read_99th_latency_us'},
...
def get_scenario_labels_from_row(row):
    key = (row['recordcount'], row['operationcount'], row['fieldcount'], row['fieldlength'], row['readallfields'])
...
def discover_and_parse_results():
    ...
    for user_dir in os.listdir(BASE_RESULTS_DIR):
        ...
        for db_dir in os.listdir(user_path):
            ...
            if not os.path.isdir(db_path) or db_dir.startswith('.'):
                continue
            ...
            for workload_dir_name in os.listdir(db_path):
                ...
                if not os.path.isdir(workload_path) or workload_dir_name.startswith('.'):
                    continue

                for scenario_dir_name in os.listdir(workload_path):
                    scenario_path = os.path.join(workload_path, scenario_dir_name)
                    if not os.path.isdir(scenario_path) or scenario_dir_name.startswith('.'):
                        continue
                    ...
                    for filename in os.listdir(scenario_path):
                        ...
                        if file_metrics:
                            record = {
                                "user": user_name_norm, "database": database_name, "workload": workload_dir_name,
                                ...
                                "phase": file_params["phase"], "repetition": int(file_params["repetition"]),
                                "timestamp_str": file_params["timestamp"]
                            }
                            record.update(file_metrics)
...
def show_values_on_bars(axs, h_v="v", space=0.4, val_format="{:.0f}"):
    """Helper function to show values on bar charts."""
...
def plot_throughput_comparison(df_to_plot, current_user_id, current_workload_id):
    ...
    sns.barplot(x='database', y='overall_throughput', hue='scenario_short_label', data=df_to_plot, palette='viridis', dodge=True, hue_order=SCENARIO_ORDER, ax=ax)
    show_values_on_bars(ax, val_format="{:,.0f}")
    ax.set_ylim(0, df_to_plot['overall_throughput'].max() * 1.15 if not df_to_plot.empty else 100) # Padding
    ax.set_title(f'Throughput: {workload_descr}\nUser: {user_descr}', fontsize=15, weight='bold')
    ax.set_ylabel('Mean Overall Throughput (ops/sec)', fontsize=12); ax.set_xlabel('Database', fontsize=12)
    ax.tick_params(axis='x', labelsize=10); ax.tick_params(axis='y', labelsize=10)
    ax.legend(title='Scenario', bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=9, title_fontsize=10)
    ax.grid(True, which='major', linestyle='--', linewidth=0.5); fig.tight_layout(rect=[0, 0, 0.80, 1])
...
def plot_latency_comparison(df_to_plot, current_user_id, current_workload_id):
    ...
    for pretty_name, metric_col in WORKLOAD_PRIMARY_LATENCIES[current_workload_id]:
        ...
        fig, ax = plt.subplots(figsize=(15, 8))
        sns.barplot(x='database', y=metric_col, hue='scenario_short_label', data=current_metric_df, palette='plasma', dodge=True, hue_order=SCENARIO_ORDER, ax=ax)
        show_values_on_bars(ax, val_format="{:,.0f}")
        ax.set_ylim(0, current_metric_df[metric_col].max() * 1.15 if not current_metric_df.empty else 100)
        ax.set_title(f'{pretty_name}: {workload_descr}\nUser: {user_descr}', fontsize=15, weight='bold')
        ax.set_ylabel(f'Mean {pretty_name}', fontsize=12); ax.set_xlabel('Database', fontsize=12)
        ax.tick_params(axis='x', labelsize=10); ax.tick_params(axis='y', labelsize=10)
        ax.legend(title='Scenario', bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=9, title_fontsize=10)
        ax.grid(True, which='major', linestyle='--', linewidth=0.5); fig.tight_layout(rect=[0, 0, 0.80, 1])
        user_plot_dir = os.path.join(BASE_PLOT_DIR, current_user_id)
...
def plot_latency_percentiles(df_to_plot, current_user_id, current_workload_id, current_database):
    ...
    for metric_group in WORKLOAD_LATENCY_PERCENTILES[current_workload_id]:
        ...
        # Ensure avg_col exists and has data before proceeding with this metric group for this DB
        if avg_col not in df_to_plot.columns or df_to_plot[avg_col].isna().all():
            # print(f"INFO: Avg metric {avg_col} not available for {metric_name_pretty} ({current_user_id}, {current_workload_id}, {current_database}). Skipping percentile plot.")
            continue
            
        current_metric_df = df_to_plot[df_to_plot[avg_col].notna()].copy() # Further ensure we only use rows with avg latency
        if current_metric_df.empty: continue
        ...
        if avg_col in current_metric_df.columns and current_metric_df[avg_col].notna().any():
             ...
             max_val = max(max_val, current_metric_df[avg_col].max())
        if p95_col and p95_col in current_metric_df.columns and current_metric_df[p95_col].notna().any():
            ...
            max_val = max(max_val, current_metric_df[p95_col].max())
        if p99_col and p99_col in current_metric_df.columns and current_metric_df[p99_col].notna().any():
            ...
            max_val = max(max_val, current_metric_df[p99_col].max())
        ...
        ax.set_ylabel(f'{metric_name_pretty}', fontsize=12)
...
# --- New Summary Plotting Functions ---
def plot_cpu_throughput_comparison(mean_data):
    os.makedirs(SUMMARY_PLOT_DIR, exist_ok=True)
    # Avg throughput across all scenarios for each user, db, workload
    ...
    for (db_id, workload_id), group_data in cpu_comp_data.groupby(['database', 'workload']):
        ...
        show_values_on_bars(ax, val_format="{:,.0f}")
        ax.set_ylim(0, group_data['overall_throughput'].max() * 1.15 if not group_data.empty else 100)
        workload_descr = WORKLOAD_DESCRIPTION_MAP.get(workload_id, workload_id)
        ax.set_title(f'CPU Throughput Comparison: {db_id.capitalize()} - {workload_descr}', fontsize=14, weight='bold')
        ax.set_xlabel('CPU (User)', fontsize=12); ax.set_ylabel('Mean Throughput (ops/sec, Avg over Scenarios)', fontsize=12)
        ax.tick_params(axis='x', rotation=10, labelsize=9); ax.tick_params(axis='y', labelsize=10)
...
def plot_workload_profile_per_cpu_db(mean_data_baseline):
    os.makedirs(SUMMARY_PLOT_DIR, exist_ok=True)
    # Data is already filtered for S1 baseline scenario
    for (user_id, db_id), group_data in mean_data_baseline.groupby(['user', 'database']):
        if group_data.empty or group_data['overall_throughput'].isna().all(): continue
        group_data['workload'] = pd.Categorical(group_data['workload'], categories=WORKLOAD_ORDER, ordered=True)
        group_data = group_data.sort_values('workload')
        fig, ax = plt.subplots(figsize=(12, 7))
        sns.barplot(x='workload', y='overall_throughput', data=group_data, palette='magma', hue='workload', dodge=False, legend=False, ax=ax)
        show_values_on_bars(ax, val_format="{:,.0f}")
        ax.set_ylim(0, group_data['overall_throughput'].max() * 1.15 if not group_data.empty else 100)
        user_descr = USER_CPU_MAP.get(user_id, user_id)
        ax.set_title(f'Workload Throughput Profile (S1 Baseline)\n{db_id.capitalize()} on {user_descr}', fontsize=14, weight='bold')
        ax.set_xlabel('Workload', fontsize=12); ax.set_ylabel('Mean Throughput (ops/sec, S1 Baseline)', fontsize=12)
        ax.set_xticks(ticks=range(len(WORKLOAD_ORDER)), labels=[WORKLOAD_DESCRIPTION_MAP.get(w,w) for w in WORKLOAD_ORDER], rotation=30, ha='right', fontsize=9)
...
def plot_aggregated_db_performance_per_cpu(mean_data):
    os.makedirs(SUMMARY_PLOT_DIR, exist_ok=True)
    # Avg throughput across all scenarios and workloads for each user, db
    ...
    for user_id, group_data in agg_db_data.groupby('user'):
        ...
        ax.set_ylim(0, group_data['overall_throughput'].max() * 1.15 if not group_data.empty else 100)
        user_descr = USER_CPU_MAP.get(user_id, user_id)
        ax.set_title(f'Aggregated Database Performance on {user_descr}', fontsize=14, weight='bold')
        ax.set_xlabel('Database', fontsize=12); ax.set_ylabel('Overall Mean Throughput (ops/sec, Avg All Workloads & Scenarios)', fontsize=10)
        ax.tick_params(axis='x', labelsize=10); ax.tick_params(axis='y', labelsize=10)
        ax.grid(True, axis='y', linestyle='--', linewidth=0.5);
...
def plot_scenario_impact_detailed(mean_data):
    # Plots go into user-specific directories as they are detailed per user/db/workload
    for (user_id, db_id, workload_id), group_data in mean_data.groupby(['user', 'database', 'workload']):
        ...
        fig, ax = plt.subplots(figsize=(12, 7))
        sns.lineplot(x='scenario_short_label', y='overall_throughput', data=group_data, marker='o', errorbar=None, linewidth=2, ax=ax)
        ax.set_ylim(0, group_data['overall_throughput'].max() * 1.1 if not group_data.empty else 100)
        user_descr = USER_CPU_MAP.get(user_id, user_id)
        workload_descr = WORKLOAD_DESCRIPTION_MAP.get(workload_id, workload_id)
        ax.set_title(f'Scenario Impact on Throughput: {db_id.capitalize()} - {workload_descr}\nUser: {user_descr}', fontsize=14, weight='bold')
        ax.set_xlabel('Scenario', fontsize=12); ax.set_ylabel('Mean Throughput (ops/sec)', fontsize=12)
        ax.tick_params(axis='x', rotation=30, ha='right', labelsize=9); ax.tick_params(axis='y', labelsize=10)
        ax.grid(True, linestyle='--', linewidth=0.5); ax.tight_layout()
...
def plot_presentation_overall_db_winner(mean_data):
    ...
    # Grand average throughput for each database
    ...
    show_values_on_bars(ax, val_format="{:,.0f}")
    ax.set_ylim(0, overall_avg['overall_throughput'].max() * 1.15 if not overall_avg.empty else 100)
...
def plot_presentation_cpu_impact_on_dbs(mean_data):
    os.makedirs(os.path.join(SUMMARY_PLOT_DIR, "presentation"), exist_ok=True)
    # Avg throughput for each CPU and DB combination (averaged over all workloads and scenarios)
    cpu_db_avg = mean_data.groupby(['cpu_description', 'database'], as_index=False)['overall_throughput'].mean()
    # Ensure correct ordering for plot
    cpu_db_avg['database'] = pd.Categorical(cpu_db_avg['database'], categories=DB_ORDER, ordered=True)
    # Create a specific order for CPUs if needed, or rely on sorted unique values
    ...
    show_values_on_bars(ax, val_format="{:,.0f}")
    ax.set_ylim(0, cpu_db_avg['overall_throughput'].max() * 1.15 if not cpu_db_avg.empty else 100)
...
def plot_presentation_workload_sensitivity_by_db(mean_data):
    os.makedirs(os.path.join(SUMMARY_PLOT_DIR, "presentation"), exist_ok=True)
    # Avg throughput for each DB and Workload (averaged over all users and scenarios)
    ...
    db_workload_avg['workload'] = pd.Categorical(db_workload_avg['workload'], categories=WORKLOAD_ORDER, ordered=True)
    db_workload_avg = db_workload_avg.sort_values(['database', 'workload'])
    if db_workload_avg.empty or db_workload_avg['overall_throughput'].isna().all(): return

    fig, ax = plt.subplots(figsize=(13,7))
    sns.lineplot(x='workload', y='overall_throughput', hue='database', data=db_workload_avg, style='database', markers=True, dashes=False, linewidth=2.5, errorbar=None, ax=ax)
    ...
    ax.set_xticks(range(len(WORKLOAD_ORDER))); ax.set_xticklabels([WORKLOAD_DESCRIPTION_MAP.get(w,w) for w in WORKLOAD_ORDER], rotation=30, ha='right', fontsize=10)
...
def plot_presentation_scenario_scalability_by_db(mean_data):
    os.makedirs(os.path.join(SUMMARY_PLOT_DIR, "presentation"), exist_ok=True)
    # Avg throughput for each DB and Scenario (averaged over all users and workloads)
    ...
    ax.set_xlabel('Scenario', fontsize=13); ax.set_ylabel('Mean Throughput (ops/sec)\n(Avg. all Users & Workloads)', fontsize=13)
    ax.tick_params(axis='x', rotation=30, ha='right', labelsize=10); ax.tick_params(axis='y', labelsize=11)
    ax.legend(title='Database', fontsize=10, title_fontsize=11)
    ax.grid(True, linestyle='--', linewidth=0.5); fig.tight_layout()
...
def main():
    ...
    if results_df.empty:
        print("No results found or parsed. Exiting."); return

    # print(f"Successfully parsed {len(results_df)} result files.")
    # print_df_summary(results_df) # Optional summary function

    run_df = results_df[results_df['phase'] == 'run'].copy()
    if run_df.empty:
        print("No 'run' phase data found. Exiting."); return

    group_cols = ['user', 'cpu_description', 'database', 'workload', 'scenario_short_label', 'scenario_full_descr',
                  'recordcount', 'operationcount', 'fieldcount', 'fieldlength', 'readallfields']
    ...
    print(f"Data prepared for plotting. Mean run data shape: {mean_run_df.shape}")

    # --- Main Plotting Loop ---
    print("\n--- Generating All Plots ---")
    unique_users = sorted(mean_run_df['user'].unique())
    # Use WORKLOAD_ORDER and DB_ORDER to guide plot generation if data exists
    
    for user_id in unique_users:
        print(f"\n>>> Processing plots for USER: {USER_CPU_MAP.get(user_id, user_id)} <<<")
        ...
        for workload_id in WORKLOAD_ORDER:
            ...
            print(f"  --- Workload: {WORKLOAD_DESCRIPTION_MAP.get(workload_id, workload_id)} ---")
            
            # Filter for throughput data specifically for this plot
            ...
            plot_latency_comparison(workload_data, user_id, workload_id) # Function itself checks for relevant metric columns
    ...
    # --- Higher-Level Summary Plots ---
    print("\n--- Generating Summary Plots ---")
    os.makedirs(SUMMARY_PLOT_DIR, exist_ok=True)

    # 1. CPU Throughput Comparison (Needs mean_run_df with all scenarios)
    print(">> Generating CPU Throughput Comparison plots...")
    plot_cpu_throughput_comparison(mean_run_df)

    # 2. Workload Performance Profile (Needs S1 baseline data from mean_run_df)
    print(">> Generating Workload Performance Profile plots...")
    baseline_scenario_label = SCENARIO_PARAMS_TO_NAME_MAP.get((10000, 10000, 10, 100, True))[0] # S1: Baseline
    ...
    if not mean_run_df_baseline_s1.empty:
        ...
        print("  INFO: No data for S1 Baseline scenario. Skipping Workload Performance Profiles.")

    # 3. Aggregated Database Performance (Needs mean_run_df with all scenarios & workloads)
    print(">> Generating Aggregated Database Performance plots...")
    plot_aggregated_db_performance_per_cpu(mean_run_df)

    # 4. Scenario Impact Detailed (Needs mean_run_df)
    print(">> Generating Scenario Impact plots (detailed per user/db/workload)...")
